...
async function refreshDashboardCounts(){
  try{
    ...
    const activeLedgerIds = new Set(ledger.map(e=>e.id));
    ...
    const saleDealPlots = (saleDealPlotsRes.data || []).filter(row=>row.plot_deal_status!=='cancelled');
    const totalSaleValue = saleDealPlots.reduce((sum,row)=>sum+Number(row.plot_sale_price||0),0);
    ...
    if(cards[1]) {
      cards[1].querySelector('h2').textContent = `${availablePlots} / ${totalPlots}`;
      cards[1].querySelector('small').textContent = `${availablePlots} available · total ${totalPlots}`;
    }
...
// -----------------------------
// Stage 12: Money comma formatting helpers
// -----------------------------
...
function formatMoneyInputValue(value){
  ...
  return Number(whole).toLocaleString('en-PK') + decimal;
}
...
function setupMoneyInputs(){
  $$('[data-money-input]').forEach(input=>{
    input.addEventListener('input',()=>{
      ...
      updateSaleWorkflowRemainingPreview();
    });
...
function updateSaleWorkflowRemainingPreview(){
  const saleInput = $('#wfSalePrice');
  const paymentInput = $('#wfPaymentAmount');
  const preview = $('#wfRemainingPreview');
  if(!saleInput || !paymentInput || !preview) return;
  const salePrice = parseMoney(saleInput.value);
  const payment = parseMoney(paymentInput.value);
  if(!salePrice){
    preview.textContent = 'Remaining will show after sale price/payment are entered.';
    return;
  }
  const remaining = Math.max(salePrice - payment, 0);
  preview.textContent = `Remaining receivable: ${formatCurrency(remaining)}`;
  if(typeof updateWorkflowPaymentStatusPreview === 'function') updateWorkflowPaymentStatusPreview();
}
...
async function openSaleWorkflow(prefill={}){await populateSaleWorkflowSelects();const f=saleWorkflowElements();f.form.reset();f.dealDate.value=todayIsoDate();f.plotStatus.value='reserved';
  if($('#wfFullyPaid')) $('#wfFullyPaid').checked=false;f.message.textContent='';if(prefill.projectId)f.projectId.value=prefill.projectId;updateWorkflowAvailablePlots();if(prefill.plotId)f.plotId.value=prefill.plotId;f.overlay.classList.remove('hidden');f.clientNameEn.focus()}
...
async function saveClientDue(e){
  ...
  const payload={
    due_type:f.dueType.value.trim()||'other',
    project_id:deal?.project_id||null,
